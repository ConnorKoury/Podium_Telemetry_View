...
function eventAddresses(eventDeviceId: string): string[] {
    return [
...
export function useTelemetry(eventInfo: ParsedEventInfo | null) {
    ...
    const registerForDevice = useCallback(
        (eventDeviceId: string) => {
            ...
            const previousDeviceId = registeredRef.current;
            if (previousDeviceId) {
                for (const address of eventAddresses(previousDeviceId)) {
                    send({ type: "unregister", address, headers: {} });
                }
            }
            registeredRef.current = eventDeviceId;
            for (const address of eventAddresses(eventDeviceId)) {
    ...
    const connect = useCallback(() => {
        ...
        function tryNext() {
            ...
            ws.onopen = () => {
                ...
                const device =
                    eventInfoRef.current?.eventDeviceId ??
            ...
            ws.onmessage = (event) => {
                ...
                try {
                    msg = JSON.parse(event.data as string) as Record<
                        string,
                        unknown
                    >;
                } catch {
                ...
                if (msgType === "err") {
                    setState((s) => ({
                        ...
                        lastError:
                            ...
                            `EventBus error: ${msg.failureType}`,
                    }));
                    return;
                }
            ...
            ws.onclose = (ev) => {
                ...
                if (connected) {
                    setState((s) => ({
                        ...s,
                        connectionState:
                            ev.code === 1000 ? "disconnected" : "error",
                        lastError:
                            ev.code !== 1000
        ...
        function handleMessage(address: string, body: unknown) {
            ...
            if (address.startsWith("sensorData.")) {
                ...
                setState((s) => ({
                    ...
                    recentPackets: [packet, ...s.recentPackets].slice(
                        0,
                        MAX_PACKETS,
                    ),
    ...
    useEffect(() => {
        if (eventInfo?.eventDeviceId) {
            registerForDevice(eventInfo.eventDeviceId);
        }
    }, [eventInfo?.eventDeviceId, registerForDevice]);
