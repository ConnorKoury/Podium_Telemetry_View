            ...
            {info && (
                <div className="bg-nova-panel border border-nova-border rounded p-3 flex flex-col gap-1.5">
                    <div className="flex items-center gap-2">
                        <span className="text-xs text-nova-dim uppercase tracking-widest">
                            Event
                        </span>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                        ...
                        <Field
                            label="EventDevice ID"
                            value={info.eventDeviceId ?? "—"}
                        />
                        <Field
                            label="Sensors"
                            value={`${info.sensorList.length} channels`}
                        />
                    </div>
                    ...
                    {info.sensorList.length === 0 && (
                        <p className="text-xs text-yellow-500 bg-yellow-950/30 border border-yellow-900/50 rounded px-2 py-1 mt-1">
                            sensorList not found in page. Sensor decoding will
                            be unavailable — raw values will still be shown.
                        </p>
                    )}

                    {!info.eventDeviceId && (
                        <p className="text-xs text-yellow-500 bg-yellow-950/30 border border-yellow-900/50 rounded px-2 py-1">
                            eventDeviceId not found. Will attempt to discover
                            via listTelemetryStreamSessions.
                        </p>
                    )}
                </div>
            )}
