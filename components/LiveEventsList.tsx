...
function SessionCard({ session, onLoad, onDirectConnect }: SessionCardProps) {
  ...
  const handleLoad = async () => {
    ...
    try {
      if (url) {
        ...
        if (!res.ok || data.error) { setError(data.error ?? "Parse failed"); return; }
        onLoad(data);
      } else {
  ...
  return (
    <div className={`rounded-lg border p-3 flex flex-col gap-2 transition-colors ${
      isNova ? "border-nova-red/40 bg-nova-red/5" : "border-nova-border bg-nova-panel"
    }`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {session.active !== undefined && (
            <span className={`flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded uppercase tracking-wider ${
              session.active ? "bg-green-900/50 text-green-400" : "bg-nova-muted/20 text-nova-dim"
            }`}>
              {session.active && <span className="blink inline-block w-1.5 h-1.5 rounded-full bg-green-400" />}
      ...
      {url && <p className="text-[10px] text-nova-muted font-mono truncate">{url}</p>}
...
export default function LiveEventsList({
  ...
  const displayed = filter === "nova" ? sessions.filter(isNovaRacing) : sessions;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex rounded-lg overflow-hidden border border-nova-border text-xs">
          ...
          >
            <span className={`inline-block w-2 h-2 rounded-sm ${filter === "nova" ? "bg-white/70" : "bg-nova-red/60"}`} />
      ...
      {!loading && filter === "nova" && novaCount === 0 && sessions.length > 0 && (
        <div className="text-center py-6 text-nova-dim text-sm">
          ...
          <button onClick={() => setFilter("all")} className="ml-1 text-nova-red underline underline-offset-2">
